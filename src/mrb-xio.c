...
void PktHandler(void)
{
	...
	else if ('X' == rxBuffer[MRBUS_PKT_TYPE]) 
	{
		...
		// Command Packet
		if ('S' == rxBuffer[MRBUS_PKT_SUBTYPE] && rxBuffer[MRBUS_PKT_LEN] >= 9 )
		{
			xioSetIO(&xio0, rxBuffer[MRBUS_PKT_SUBTYPE+1], rxBuffer[MRBUS_PKT_SUBTYPE+2]);
		}
		else if ('O' == rxBuffer[MRBUS_PKT_SUBTYPE] && rxBuffer[MRBUS_PKT_LEN] >= 11 )
		{
			for(i=0; i<5; i++)
			{
				xio0.io[i] = (xio0.io[i] & xio0.direction[i]) | (rxBuffer[MRBUS_PKT_SUBTYPE + 1 + i] & !(xio0.direction[i]));
			}
		}
...
int main(void)
{
	...
	// Initialize MRBus address from EEPROM address 0x20
		...
		uint8_t i;
		for(i=0; i<5; i++)
			xio0_directions[i] = eeprom_read_byte((uint8_t*)MRBXIO_EE_XIO0_DDR_IO0 + i);
